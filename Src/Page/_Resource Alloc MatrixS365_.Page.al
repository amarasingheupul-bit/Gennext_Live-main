page 50104 "Resource Alloc MatrixS365"
{
    Caption = 'Transport Capacity Details';
        ...
        area(Processing)
        {
            action(LoadData)
            {
                ...
                begin
                    ...
                    LoadMatrix(JobRec, HorizontalRecord, MatrixColumnCaptions, MatrixRecords, AmountType);
                end;
            }
        }
    }
    ...
    begin
        ...
        MatrixRec.SetRange("No.", ResourceFilter);
        if Rec.GetFilter("Resource Filter") <> '' then MatrixRec.SetFilter("No.", Rec.GetRangeMin("Resource Filter"), Rec.GetRangeMax("Resource Filter"));
        for I := 1 to ArrayLen(MatrixColumnDateFilters) do begin
            ...
            if MatrixRec.Find('-') then repeat MatrixCellQuantity += Round(MatrixRec.Quantity * MatrixRec."Qty. per Unit of Measure", UOMMgt.QtyRndPrecision()) until MatrixRec.Next() = 0;
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        JobPlanningLine.SetFilter("No.", ResourceFilter);
    ...
    begin
        HorizontalRecord.SetRange("No.", ResourceNo);
        HorizontalRecord.SetRange(Type, HorizontalRecord.Type::Resource);
        JobRec.SetRange("Resource Filter", ResourceNo);
        OnActionShowMatrixOnAfterSetJobFilters(JobRec);
        LoadMatrix(JobRec, HorizontalRecord, MatrixColumnCaptions, MatrixRecords, AmountType);
        CurrPage.Update();
    end;

    procedure SetResouceNo(ResNo: Code[20])
    begin
        ResourceNo := ResNo;
    end;

    procedure GetResouceNo(): Code[20]
    begin
        exit(ResourceNo);
    end;
