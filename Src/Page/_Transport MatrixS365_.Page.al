page 50106 "Transport MatrixS365"
{
    ...
    SourceTableView = where("Resource Group No." = const('TRANSPORT'));
    UsageCategory = Lists;
    Editable = true;
        ...
        area(content)
        {
            group(General)
            {
                ...
                field(ResourceFilter; ResourceFilter)
                {
                    ...
                    trigger OnValidate()
                    begin
                        DateFilter := Format(CalcDate('-<CW>', Today)) + '..' + Format(CalcDate('<CW>', Today));
                        UpdateContent();
                    end;
                }
    ...
    actions
    {
        area(Processing)
        {
            action(ShowDetails)
            {
                ...
                trigger OnAction()
                var
                    ResourceAllMatrix: page "Resource Alloc MatrixS365";
                begin
                    ResourceAllMatrix.SetResouceNo(Rec."No.");
                    ResourceAllMatrix.RunModal();
                end;
            }
        }
    }
    ...
    procedure LoadMatrix(var NewVerticalRec: Record Resource; var NewHorizontalRec: Record "Job Planning Line"; NewMatrixColumnCaptions: array[32] of Text[100]; var NewMatrixDateFilters: array[32] of Record Date; AmountType2: Enum "Analysis Amount Type")
    begin
        if Rec.IsEmpty then Rec.Copy(NewVerticalRec);
    ...
    begin
        MatrixRec.Reset();
        MatrixRec.SetCurrentKey("Job No.");
        MatrixRec.SetRange("No.", Rec."No.");
        MatrixRec.SetRange(Type, MatrixRec.Type::Resource);
        //MatrixRec.SetRange("Schedule Line", true);
        if Rec.GetFilter("No.") <> '' then MatrixRec.SetFilter("No.", Rec.GetRangeMin("No."), Rec.GetRangeMax("No."));
        for I := 1 to ArrayLen(MatrixColumnDateFilters) do begin
            MatrixCellQuantity := 0;
            MatrixRec.SetRange("Planning Date", GetStartDate(MatrixColumnDateFilters[I]."Period Start"), MatrixColumnDateFilters[I]."Period End");
            OnMatrixOnAfterGetRecordOnBeforeMatrixRecLoop(MatrixRec, MatrixCellQuantity);
            if MatrixRec.Find('-') then
                repeat
                    if JobNo <> MatrixRec."Job No." then MatrixCellQuantity += 1; //Round(MatrixRec.Quantity * MatrixRec."Qty. per Unit of Measure", UOMMgt.QtyRndPrecision())
                    JobNo := MatrixRec."Job No.";
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        // JobPlanningLine.SetFilter("No.", Rec.GetRangeMin("No."), Rec.GetRangeMax("No."));
        JobPlanningLine.SetRange("Planning Date", GetStartDate(MatrixColumnDateFilters[Column]."Period Start"), MatrixColumnDateFilters[Column]."Period End");
        JobPlanningLine.SetRange("No.", Rec."No.");
        JobPlanningLine.SetRange(Type, JobPlanningLine.Type::Resource);
        if JobPlanningLine.FindSet() then
            repeat
                if Job.get(JobPlanningLine."Job No.") then job.Mark(true);
            until JobPlanningLine.Next() = 0;
        //if JobPlanningLine.FindFirst() then
        // Job.SetFilter("No.", JobPlanningLine.GetRangeMin("Job No."), JobPlanningLine.GetRangeMax("Job No."));
    ...
    begin
        MatrixMgt.GeneratePeriodMatrixData(StepType.AsInteger(), 32, false, PeriodType, DateFilter, PKFirstRecInCurrSet, MatrixColumnCaptions, ColumnsSet, CurrSetLength, MatrixRecords);
    end;

    local procedure UpdateContent()
    begin
        // HorizontalRecord.SetFilter("No.", Rec.GetRangeMin("No."), Rec.GetRangeMax("No."));
        HorizontalRecord.SetRange(Type, HorizontalRecord.Type::Resource);
        // ResourceRec.SetFilter("No.", Rec.GetRangeMin("No."), Rec.GetRangeMax("No."));
        OnActionShowMatrixOnAfterSetJobFilters(ResourceRec);
        LoadMatrix(ResourceRec, HorizontalRecord, MatrixColumnCaptions, MatrixRecords, AmountType);
        CurrPage.Update();
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnActionShowMatrixOnAfterSetJobFilters(var ResourceRec: Record Resource)
    begin
    end;
}
